...
def test_clean_pear_run_validates(tmp_path: Path) -> None:
    ...
    report = validate_runs([tmp_path], allow_mock=True)

    assert report.ok, [i.render() for i in report.errors()]
    assert report.mock_status == "mock"
...
def test_unsupported_schema_version_can_be_opted_into(tmp_path: Path) -> None:
    ...
    rejected = validate_runs([tmp_path], allow_mock=True)
    accepted = validate_runs([tmp_path], allow_mock=True, allowed_schema_versions={2, 3})
...
# --------------------------------------------------------------------- mock --
def test_mock_runs_need_allow_mock(tmp_path: Path) -> None:
    make_pear_run(tmp_path / "exp", "mock_run")

    assert "mock_not_allowed" in error_codes(validate_runs([tmp_path]))
    assert validate_runs([tmp_path], allow_mock=True).ok


def test_mixed_mock_and_real_is_rejected_even_with_allow_mock(tmp_path: Path) -> None:
    root = tmp_path / "exp"
    make_pear_run(root, "mock_run", mock=True)
    make_pear_run(root, "real_run", mock=False)

    report = validate_runs([root], allow_mock=True)

    assert report.mock_status == "mixed"
    assert "mock_and_real_mixed" in error_codes(report)


def test_mock_flag_inconsistent_within_a_run(tmp_path: Path) -> None:
    run = make_pear_run(tmp_path / "exp", "inconsistent", mock=True)
    patch_summary(run, mock=False)

    report = validate_runs([tmp_path], allow_mock=True)

    assert "mock_flag_inconsistent" in error_codes(report)
...
def test_cli_exit_codes(tmp_path: Path, capsys) -> None:
    ...
    assert main([str(tmp_path), "--allow-mock"]) == 0
    assert main([str(tmp_path)]) == 1
    out = capsys.readouterr().out
    assert "mock status" in out
...
def test_cli_writes_json_report(tmp_path: Path, capsys) -> None:
    ...
    assert main([str(tmp_path), "--allow-mock", "--json", str(out_path)]) == 0
